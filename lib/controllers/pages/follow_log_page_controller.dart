...
part 'follow_log_page_controller.freezed.dart';

@freezed
class FollowLogPageState with _$FollowLogPageState {
  const FollowLogPageState._();

  const factory FollowLogPageState(
      ...
      @Default(true) bool isAddClothes}) = _FollowLogPageState;
}

final followLogPageProvider =
    StateNotifierProvider<FollowLogPageController, FollowLogPageState>((ref) {
  final user = ref.watch(userProvider.select((value) => value.user));
  return FollowLogPageController(
    ref.read,
    userId: user.uid,
  );
});

class FollowLogPageController extends StateNotifier<FollowLogPageState> {
  FollowLogPageController(
    this._read, {
    required String userId,
  })  : _userId = userId,
        super(FollowLogPageState()) {
    if (_userId != '') {
      fetchTimeLine();
    }
  }
  ...
  Future<void> fetchTimeLine() async {
    ...
    final logList = await _read(timeLineRepositoryProvider)
        .fetchFollowTimeLines(userId: _userId);
  ...
  Future<void> endScroll() async {
    ...
    final addLogList = await _read(timeLineRepositoryProvider)
        .fetchAddFollowTimeLines(lastItemId: lastItemId, userId: _userId);
    final logMap = {...state.logMap};
