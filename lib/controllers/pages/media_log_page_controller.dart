...
final MediaLogPageProvider =
StateNotifierProvider.autoDispose<MediaLogPageController, MediaLogPageState>(
        (ref) {
      final user = ref.watch(userProvider.select((value) => value.user));
      return MediaLogPageController(ref.read, userId: user.uid, );
    });


class MediaLogPageController extends StateNotifier<MediaLogPageState> {
  MediaLogPageController(this._read,{required String userId,})  :
        _userId = userId, super(MediaLogPageState()){
    if(_userId != '') {
      _init();
    }
  }
  ...
  Future<void> _init() async{
    final timeLineList = await _read(timeLineRepositoryProvider).fetchShares(userId: _userId, genre: 'media');
  ...
  Future<void> endScroll()async{
    ...
    final addShares = await _read(timeLineRepositoryProvider).fetchAddShares(userId: _userId, lastItemId: lastItemId, genre: 'media');
