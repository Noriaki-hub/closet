...
class ShopTimeLineList extends HookConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final timeLineList = ref.watch(
        ShopLogPageProvider.select((value) => value.timeLineList));
    return timeLineList.isEmpty ? Center(
      child: Text('現在ログはありません', style: TextStyle(color: Colors.black45),),) :
    ListView.builder(
                  ...
                  children: [
                    Row(
                      children: [
                        ...
                        Text(
                          'シェアしました', style: TextStyle(color: Colors.black45),)
                      ],
                    ),
                    timeLine.uid != userId ? Container() :
                  ...
                  onTap: () {
                    Navigator.push(
                        ...
                        MaterialPageRoute(
                          builder: (context) =>
                              WebViewScreen(
                                genre: timeLine.genre, url: timeLine.url,),
                        )
                    );
                  },
                  child: ListTile(
